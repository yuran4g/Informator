...
public class NewArchiver extends JFrame {
    ...
    private JLabel createInformatorButton() {
        if (informator == null)
            informator = new InformatorButton(0, rows.size() > 0 ? (Row.HEIGHT * rows.size() + Row.TextAndIconSize / 2) : (20 + Row.TextAndIconSize / 2));
        else
            informator.setLocation(0, rows.size() > 0 ? (Row.HEIGHT * rows.size() + Row.TextAndIconSize / 2) : (20 + Row.TextAndIconSize / 2));
        return informator;
    }
    ...
    private class minimizeMouseListener implements MouseListener {
        public void mouseClicked(MouseEvent e) {
            ...
            try {
                ...
                logger.error("Can not add trayIcon");
            }
        }
